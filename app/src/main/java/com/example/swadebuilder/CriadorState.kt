...
class CriadorState {
    ...
    companion object {
        ...
        val mapaDescricoesSignos = mapOf(
    ...
    fun valorAparar(): Int {
        ...
        val garcaBonus = if (signoAdgSelecionado == "Garça") 1 else 0

        return base + bloquearBonus + bloquearAprimoradoBonus + bonusApararFromPower + garcaBonus
    }
    ...
    val reservaChi by derivedStateOf {
        ...
        val bonusKirin = if (signoAdgSelecionado == "Kirin") 1 else 0

        // Base 2 added as requested
        val baseChi = if (compendioArteDaGuerraAtivo) 2 else 0

        (baseChi + espiritoRaw / 2 - racialPenalty + bonusFromChiEdges + bonusFromTropo + bonusKirin).coerceAtLeast(0)
    }
    ...
    private fun atributoBaseRacial(a: String): Int {
        ...
        if (signoAdgSelecionado != null) {
            ...
            when (signoAdgSelecionado) {
                "Boi" -> if (key == "FORCA") signBonus = 2
                "Dragão" -> if (key == "ESPIRITO") signBonus = 2
                "Macaco" -> if (key == "ASTUCIA") signBonus = 2
                "Urso" -> if (key == "VIGOR") signBonus = 2
            }
        }

        val racialAndSign = base + signBonus

        return if (a.keyify() == "AGILIDADE" && meioElfoAgil) {
            maxOf(racialAndSign, 6)
        } else {
            racialAndSign
        }
    }
    ...
    fun periciaStartRaw(anc: String, per: Pericia): Int {
        ...
        val base = racialSkillStartMap[ancKey]?.get(perKey)
            ?: if (per.basica) 4 else 0

        // Se Arte da Guerra não ativo ou não tiver signo, retorna base
        if (!compendioArteDaGuerraAtivo || signoAdgSelecionado == null) return base

        // Lógica de Signos
        // Lebre: Cura d6 (raw 6)
        if (signoAdgSelecionado == "Lebre" && perKey == "CURA") {
             return maxOf(base, 6)
        }
        // Garça: Acrobacia d4 (raw 4)
        if (signoAdgSelecionado == "Garça" && perKey == "ACROBACIA") {
             return maxOf(base, 4)
        }
        // Serpente: Jogar ou Performance d6
        if (signoAdgSelecionado == "Serpente") {
             val chosen = signoSerpentePericiaEscolhida.keyify() // "JOGAR" ou "PERFORMANCE"
             if (perKey == chosen) {
                  return maxOf(base, 6)
             }
        }
    ...
    fun selecionarSigno(novoSigno: String) {
        ...
        // Remover vantagens do signo anterior se estavam em vantagensAutomaticas
        val prevSigno = signoAdgSelecionado
        if (prevSigno != null) {
            val edgeToRemove = when (prevSigno) {
                "Basabasa" -> "atraente"
                "Raposa" -> "elevar_a_moral"
                "Lobo" -> "elo_comum"
                else -> null
            }
            if (edgeToRemove != null) {
                val removed = vantagensSelecionadas.removeAll { it.id == edgeToRemove }
                if (removed) {
                    vantagensAutomaticas.removeIf { it.keyify() == edgeToRemove.keyify() }
                }
            }
        }
        ...
        // Adicionar vantagem do novo signo
        val newEdgeId = when (novoSigno) {
            "Basabasa" -> "atraente"
            "Raposa" -> "elevar_a_moral"
            "Lobo" -> "elo_comum"
            else -> null
        ...
        if (newEdgeId != null) {
             val vant = listaVantagens.firstOrNull { it.id == newEdgeId }
             if (vant != null && vantagensSelecionadas.none { it.id == newEdgeId }) {
                 vantagensSelecionadas.add(vant)
                 vantagensAutomaticas.add(vant.nome)
             }
        }
