        ...
        Column(modifier = containerModifier) {
            ...
            if (!emProgresso && !usaRequisicao) {
                if (usePbWalletRedesign) {
                    ...
                    ResourceControlRow(
                        labelAdd = "Usar PB em Recursos",
                        labelRemove = "Desfazer uso de PB",
                        canAdd = pcLivres > 0 && recursosPcUsados == 0,
                        canRemove = recursosPcUsados > 0,
                        onAdd = onUsarPontosBonusEmRecursos,
                        onRemove = onDesfazerPontosBonusEmRecursos
                    )
                }
                Spacer(Modifier.size(8.dp))
            }
